...
- PATH nodes carry a \`svgKey\` field (format: S{sectionIndex}:{name}|{nodePath}). The SVG markup is NOT in the DSL. Place \`@@SVG:{svgKey}@@\` where each icon goes, then call \`mcp__applyDesign\` at the end to inject the real high-precision SVG. NEVER hand-write \`<path d="...">\`.
