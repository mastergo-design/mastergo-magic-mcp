...
const buildDslRules = (): string[] => {
  return [
    ...
    "CRITICAL — SVG STRIPPED sections: when hasStrippedSvgs=true, the section's PATH nodes have empty data arrays and the icons in dsl.icons use @svgCache: prefix. You MUST call mcp__getDesignSvgs to fetch the actual SVG HTML. NEVER hand-draw or guess stripped SVGs.",
    "CRITICAL — SVG KEY MATCHING: dsl.icons[name] maps semantic name → @svgCache reference. getDesignSvgs returns `resolvedIcons` — a two-level index `{sectionIndex: {iconName: svgKey}}`. Look up icons via DIRECT dictionary access: `resolvedIcons[\"<sectionIndex>\"][\"<iconName>\"]` → svgKey → `svgs[svgKey]` = svgHtml. This is ZERO string matching. Example: `resolvedIcons[\"1\"][\"White\"]` → `\"S1:White|1:5449/1:5041\"` → `svgs[\"S1:White|1:5449/1:5041\"]`. Render EVERY icon from dsl.icons — do NOT skip any.",
    "CRITICAL — `rowTexts` items now carry `parentType`/`parentName` fields: use these to determine what each text belongs to. A \"8\" inside a button INSTANCE is that button's badge count, NOT an independent notification badge. Do NOT add notification badges to buttons that already contain their own count text.",
    ...
    "CRITICAL — BACKGROUND COLOR FROM splitContainers: splitContainers[].background IS the exact CSS background-color value. Copy it verbatim: `background-color: <value>;`. MUST NOT add gradients, MUST NOT change the color, MUST NOT add transparency or effects. The designer chose this precise color. This overrides any aesthetic preference.",
    "CRITICAL — OPACITY ON FRAME: when a FRAME/GROUP node has an `opacity` field (e.g. opacity=0.5), it means the frame's OWN background/fill is translucent, NOT its children. Translate this as `background-color: rgba(R, G, B, <opacity>)` or `background: rgba(...)` on the frame itself. NEVER use CSS `opacity: X` on the parent — that would make child text/icons translucent too. Compute the rgba from the frame's fill color + opacity value.",
    ...
    "CRITICAL — SVG icons, NOT text: table action columns (操作列) use SVG icon buttons (edit/delete), NOT plain text. Use resolvedIcons[sectionIndex][iconName] → svgs[key] for direct SVG lookup. Insert the exact svgHtml. NEVER render action buttons as `<p>编辑</p>` or `<p>删除</p>` plain text. Sidebar menu icons also use SVG — do NOT draw simplified placeholder shapes when the real SVG data is available from getDesignSvgs.",
