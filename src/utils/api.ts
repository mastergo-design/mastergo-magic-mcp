...
const buildDslRules = (): string[] => {
  return [
    ...
    "CRITICAL — SVG KEY MATCHING: dsl.icons[name] maps semantic name → @svgCache reference (e.g. \"White\" → \"@svgCache:1:White\"). getDesignSvgs returns keys in format \"S<sectionIndex>:<name>|<nodePath>\" (e.g. \"S1:White|1:5449/1:5041\"). Match by: extract the icon name from dsl.icons key, find getDesignSvgs key that starts with \"S<sectionIndex>:<iconName>|\". Example: dsl.icons[\"White\"]=\"@svgCache:1:White\" → find key \"S1:White|...\" in getDesignSvgs response. Render EVERY icon from dsl.icons — do NOT skip any.",
