...
将 `{domain}`、`{fileId}`、`{layerId}` 替换为实际值。也可以使用短链接：
...
```
npx @mastergo/magic-mcp --token=YOUR_TOKEN [--url=API_URL] [--rule=RULE_NAME] [--proxy=PROXY_URL] [--header=Key: Value] [--debug] [--no-rule]
```
